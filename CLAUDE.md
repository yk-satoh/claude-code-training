...
## 会社情報を扱うページ（/ndb-renewal等）を実装・レビューする時のルール

`/ndb-renewal` はTask13で作成した研修用のコーポレートサイト改善モックであり、本番サイトではない。このようなページを実装・レビューする際は以下を守る。

- 既存サイトのHTML/CSS/JavaScriptはコピーせず、公開されている会社情報のみを参考にする
- 実績数値、取引社数、資格者数、独自の制度名など出典のない情報は創作しない（モックデータを使う場合は研修用モックである旨をコード上に示す）
- 文言を調整する場合も、会社名・サービス名など会社情報の「意味」自体は変えない
- UIレビューでは、レスポンシブ崩れ、アクセシビリティ（見出し構造、ボタン/リンクの使い分け、カラーコントラスト、`<main>`等のランドマーク）、CTA文言と遷移先の整合性を確認する
- テストはユーザーから見える文言・見出し・導線（リンクの`href`と対応する`id`など）を中心に検証し、新規依存を増やさず既存構成（Vitest + happy-dom、`renderToStaticMarkup`によるHTML検証）に合わせる
- 似た構造のセクションは共通コンポーネント化し、スタイルはCSS Modules + CSS変数で統一するなど、ジュニアエンジニアが読みやすい構成を意識する
